...
class _FollowUpsFilterSheet extends ConsumerStatefulWidget {
  ...
  @override
  ConsumerState<_FollowUpsFilterSheet> createState() => _FollowUpsFilterSheetState();
}
...
class _FollowUpsFilterSheetState extends ConsumerState<_FollowUpsFilterSheet> {
  ...
  late bool _useRange = widget.current?.dueDateFrom != null || widget.current?.dueDateTo != null;
  ...
  Future<void> _pickDate({required bool isFrom}) async {
    final date = await showDatePicker(
      ...
      initialDate: (_useRange ? (isFrom ? _fromDate : _toDate) : _singleDate) ?? DateTime.now(),
    );
    if (date == null) return;
    setState(() {
      if (_useRange) {
            ...
            children: [
              Row(
                children: [
                  ...
                  TextButton(onPressed: _showAll, child: const Text('Show All')),
              ...
              DropdownButtonFormField<(String, String)>(
                ...
                items: [
                  for (final (sortBy, sortOrder, label) in _sortOptions)
                    DropdownMenuItem(value: (sortBy, sortOrder), child: Text(label)),
                ],
                onChanged: (value) {
              ...
              if (branchesAsync.valueOrNull != null && branchesAsync.valueOrNull!.isNotEmpty) ...[
              ...
              Row(
                children: [
                  Expanded(child: Text('Due Date', style: theme.textTheme.labelLarge)),
                  Row(
                    mainAxisSize: MainAxisSize.min,
                    children: [
                      const Text('Range'),
              ...
              if (_useRange)
                  ...
                  children: [
                    Expanded(
                      child: OutlinedButton(
                        ...
                        child: Text(_fromDate == null ? 'From' : '${_fromDate!.toLocal()}'.split(' ').first),
                      ),
                    ),
                    ...
                    Expanded(
                      child: OutlinedButton(
                        ...
                        child: Text(_toDate == null ? 'To' : '${_toDate!.toLocal()}'.split(' ').first),
                      ),
                    ),
                  ],
                )
                ...
                OutlinedButton(
                  onPressed: () => _pickDate(isFrom: true),
                  child: Text(_singleDate == null ? 'Select a date' : '${_singleDate!.toLocal()}'.split(' ').first),
                ),
