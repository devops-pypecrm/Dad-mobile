...
class _EditLeadScreenState extends ConsumerState<EditLeadScreen> {
  ...
  late final _firstNameController = TextEditingController(text: widget.lead.firstName);
  late final _lastNameController = TextEditingController(text: widget.lead.lastName ?? '');
  ...
  late final _secondaryPhoneController = TextEditingController(text: widget.lead.secondaryPhone ?? '');
  late final _companyController = TextEditingController(text: widget.lead.company ?? '');
  late final _enquiryAboutController = TextEditingController(text: widget.lead.enquiryAbout ?? '');
  ...
  Future<void> _submit() async {
    ...
    if (kProductGatedStatuses.contains(_status) && !widget.lead.hasProducts && _status != widget.lead.status) {
      showDialog(
        context: context,
    ...
    final updated = await ref.read(editLeadControllerProvider(widget.lead.id).notifier).submit(
